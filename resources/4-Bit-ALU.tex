...
\begin{document}
    \begin{tikzpicture}
        ...
        \draw (C.bpin  2) -| (A.bpin 2);
        % \draw (C.bpin 63) -- ++(0,-0.25) -| (A.bpin 1);
        \draw (C.bpin 1) -| (A.bpin 1);
        %
        \draw (C.bpin 50) node [anchor=north] () {\tiny \ttfamily Bottom View};
        \draw (C.bpin  3) node [anchor=south west] () {\tiny \ttfamily uio[2]};
        ...
        \draw (C.bpin 1) node [anchor=south west] () {\tiny \ttfamily uio[0]};
        \draw (A.bpin 1) node [anchor=north, rotate=45] () {\tiny \ttfamily ~~~~~opcode[0]};
        %\draw (A.bpin 2) node [anchor=south east, rotate=45, xshift=1cm] () {\tiny \ttfamily ~~~~~opcode[1]};
        \draw (A.bpin 3) node [anchor=south east, rotate=45] () {\tiny \ttfamily opcode[2]};
        %
        ...
        %
        \draw(C.center) ++(3,0) node[](){\includegraphics[width=2cm]{Logo-EN.pdf}};
    \end{tikzpicture}
\end{document}
